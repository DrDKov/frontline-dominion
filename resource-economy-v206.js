(() => {
  ...
  function financialIncome206(game, dt) {
    ...
    for (const teamKey of ['player','enemy']) {
      ...
      const centers = (game.buildings || []).filter(b => b?.alive && b.completed && b.team === teamKey && b.sabotagedUntil <= game.time &&
        ['financialCenter','financialTradeCenter','creditExchange','commodityExchange','industrialCommercialCenter','logisticsCommercialTerminal'].includes(b.typeId));
  ...
  function importCycles206(game) {
    ...
    for (const building of (game.buildings || []).filter(b => b?.alive && b.completed && L.ensureNode(b)?.nodeType === 'trade')) {
