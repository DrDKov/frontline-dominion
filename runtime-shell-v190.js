...
(() => {
  ...
  const BUILD = 190;
  if (globalThis.__FD_RUNTIME_SHELL_190__) return;

  const root = document.documentElement;
  root.dataset.fdBuild = String(BUILD);
  root.dataset.fdVersion = VERSION;

  const boot = globalThis.__FD_BOOT_190__;
  const state = {
    installed: false,
    launching: false,
    launchMode: null,
  ...
  const saveKeys190 = api => {
    ...
    const backup = `${current}-backup-build190`;
    return {
      current,
      backup,
      candidates: [...new Set([
  ...
  const showLaunchError = (text) => {
    ...
    console.error('[FD190] Launch error:', state.lastError);
  ...
  const safeSave190 = (reason = 'periodic') => {
    ...
    try {
      ...
      console.info('[FD190] Checkpoint saved:', reason);
      return true;
    } catch (error) {
      if (parseSave190(previous, keys.current)) storageSet190(api, keys.current, previous);
      console.warn('[FD190] Checkpoint save failed:', reason, error);
      return false;
    }
  };
  ...
  const finishSuccessfulLaunch = () => {
    ...
    boot?.stop?.();
    root.classList.remove('fd-loading190', 'fd-ready190');
    root.classList.add('fd-running190');
    state.launching = false;
    const mode = state.launchMode;
    state.launchMode = null;
    configureAutosave190();
    console.info(
      '[FD190] Stable game observed after launch',
  ...
  const launchNewGame = (event) => {
    ...
    console.info('[FD190] Starting new game', options);
    try {
      api.startGame(options);
      verifyLaunch();
    } catch (error) {
      console.error('[FD190] startGame threw', error);
  ...
  const launchSavedGame = (event) => {
    ...
    console.info('[FD190] Starting saved game from', candidate.key);
    try {
      api.startGame({
      ...
      console.error('[FD190] saved start threw', error);
  ...
  const install = () => {
    ...
    boot?.setReady?.(true);
    console.info('[FD190] Canonical launch/save owner ready', {
  ...
  globalThis.__FD_RUNTIME_SHELL_190__ = {
    version: VERSION,
    build: BUILD,
    state,
    install,
