...
(() => {
  ...
  const BUILD = 192;
  if (globalThis.__FD_RUNTIME_SHELL_192__) return;

  const root = document.documentElement;
  root.dataset.fdBuild = String(BUILD);
  root.dataset.fdVersion = VERSION;

  const boot = globalThis.__FD_BOOT_192__;
  const state = {
    installed: false,
    launching: false,
    launchMode: null,
  ...
  const showLaunchError = (text) => {
    ...
    console.error('[FD192] Launch error:', state.lastError);
  ...
  const safeSave190 = (reason = 'periodic') => {
    ...
    try {
      ...
      console.info('[FD192] Checkpoint saved:', reason);
      return true;
    } catch (error) {
      if (parseSave190(previous, keys.current)) storageSet190(api, keys.current, previous);
      console.warn('[FD192] Checkpoint save failed:', reason, error);
      return false;
    }
  };
  ...
  const finishSuccessfulLaunch = () => {
    ...
    boot?.stop?.();
    root.classList.remove('fd-loading192', 'fd-ready192');
    root.classList.add('fd-running192');
    state.launching = false;
    const mode = state.launchMode;
    state.launchMode = null;
    configureAutosave190();
    console.info(
      '[FD192] Stable game observed after launch',
  ...
  const launchNewGame = (event) => {
    ...
    console.info('[FD192] Starting new game', options);
    try {
      api.startGame(options);
      verifyLaunch();
    } catch (error) {
      console.error('[FD192] startGame threw', error);
  ...
  const launchSavedGame = (event) => {
    ...
    console.info('[FD192] Starting saved game from', candidate.key);
    try {
      api.startGame({
      ...
      console.error('[FD192] saved start threw', error);
  ...
  const install = () => {
    ...
    boot?.setReady?.(true);
    console.info('[FD192] Canonical launch/save owner ready', {
  ...
  globalThis.__FD_RUNTIME_SHELL_192__ = {
    version: VERSION,
    build: BUILD,
    state,
    install,
