...
(() => {
  ...
  const BUILD = 195;
  if (globalThis.__FD_RUNTIME_SHELL_195__) return;

  const root = document.documentElement;
  root.dataset.fdBuild = String(BUILD);
  root.dataset.fdVersion = VERSION;

  const boot = globalThis.__FD_BOOT_195__;
  const state = {
    installed: false,
    launching: false,
    launchMode: null,
  ...
  const showLaunchError = (text) => {
    ...
    console.error('[FD195] Launch error:', state.lastError);
  ...
  const safeSave190 = (reason = 'periodic') => {
    ...
    try {
      ...
      console.info('[FD195] Checkpoint saved:', reason);
      return true;
    } catch (error) {
      if (parseSave190(previous, keys.current)) storageSet190(api, keys.current, previous);
      console.warn('[FD195] Checkpoint save failed:', reason, error);
      return false;
    }
  };
  ...
  const finishSuccessfulLaunch = () => {
    ...
    boot?.stop?.();
    root.classList.remove('fd-loading195', 'fd-ready195');
    root.classList.add('fd-running195');
    state.launching = false;
    const mode = state.launchMode;
    state.launchMode = null;
    configureAutosave190();
    console.info(
      '[FD195] Stable game observed after launch',
  ...
  const launchNewGame = (event) => {
    ...
    console.info('[FD195] Starting new game', options);
    try {
      api.startGame(options);
      verifyLaunch();
    } catch (error) {
      console.error('[FD195] startGame threw', error);
  ...
  const launchSavedGame = (event) => {
    ...
    console.info('[FD195] Starting saved game from', candidate.key);
    try {
      api.startGame({
      ...
      console.error('[FD195] saved start threw', error);
  ...
  const install = () => {
    ...
    boot?.setReady?.(true);
    console.info('[FD195] Canonical launch/save owner ready', {
