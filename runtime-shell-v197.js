...
(() => {
  ...
  const BUILD = 197;
  if (globalThis.__FD_RUNTIME_SHELL_197__) return;

  const root = document.documentElement;
  root.dataset.fdBuild = String(BUILD);
  root.dataset.fdVersion = VERSION;

  const boot = globalThis.__FD_BOOT_197__;
  const state = {
    installed: false,
    launching: false,
    launchMode: null,
  ...
  const showLaunchError = (text) => {
    ...
    console.error('[FD197] Launch error:', state.lastError);
  ...
  const safeSave190 = (reason = 'periodic') => {
    ...
    try {
      ...
      console.info('[FD197] Checkpoint saved:', reason);
      return true;
    } catch (error) {
      if (parseSave190(previous, keys.current)) storageSet190(api, keys.current, previous);
      console.warn('[FD197] Checkpoint save failed:', reason, error);
      return false;
    }
  };
  ...
  const finishSuccessfulLaunch = () => {
    ...
    boot?.stop?.();
    root.classList.remove('fd-loading197', 'fd-ready197');
    root.classList.add('fd-running197');
    state.launching = false;
    const mode = state.launchMode;
    state.launchMode = null;
    configureAutosave190();
    console.info(
      '[FD197] Stable game observed after launch',
  ...
  const launchNewGame = (event) => {
    ...
    console.info('[FD197] Starting new game', options);
    try {
      api.startGame(options);
      verifyLaunch();
    } catch (error) {
      console.error('[FD197] startGame threw', error);
  ...
  const launchSavedGame = (event) => {
    ...
    console.info('[FD197] Starting saved game from', candidate.key);
    try {
      api.startGame({
      ...
      console.error('[FD197] saved start threw', error);
  ...
  const install = () => {
    ...
    boot?.setReady?.(true);
    console.info('[FD197] Canonical launch/save owner ready', {
  ...
  globalThis.__FD_RUNTIME_SHELL_197__ = {
    version: VERSION,
    build: BUILD,
    state,
    install,
