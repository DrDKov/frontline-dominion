...
(() => {
  ...
  const BUILD = 198;
  if (globalThis.__FD_RUNTIME_SHELL_198__) return;

  const root = document.documentElement;
  root.dataset.fdBuild = String(BUILD);
  root.dataset.fdVersion = VERSION;

  const boot = globalThis.__FD_BOOT_198__;
  const state = {
    installed: false,
    launching: false,
    launchMode: null,
  ...
  const showLaunchError = (text) => {
    ...
    console.error('[FD198] Launch error:', state.lastError);
  ...
  const safeSave190 = (reason = 'periodic') => {
    ...
    try {
      ...
      console.info('[FD198] Checkpoint saved:', reason);
      return true;
    } catch (error) {
      if (parseSave190(previous, keys.current)) storageSet190(api, keys.current, previous);
      console.warn('[FD198] Checkpoint save failed:', reason, error);
      return false;
    }
  };
  ...
  const finishSuccessfulLaunch = () => {
    ...
    boot?.stop?.();
    root.classList.remove('fd-loading198', 'fd-ready198');
    root.classList.add('fd-running198');
    state.launching = false;
    const mode = state.launchMode;
    state.launchMode = null;
    configureAutosave190();
    console.info(
      '[FD198] Stable game observed after launch',
  ...
  const launchNewGame = (event) => {
    ...
    console.info('[FD198] Starting new game', options);
    try {
      api.startGame(options);
      verifyLaunch();
    } catch (error) {
      console.error('[FD198] startGame threw', error);
  ...
  const launchSavedGame = (event) => {
    ...
    console.info('[FD198] Starting saved game from', candidate.key);
    try {
      api.startGame({
      ...
      console.error('[FD198] saved start threw', error);
  ...
  const install = () => {
    ...
    boot?.setReady?.(true);
    console.info('[FD198] Canonical launch/save owner ready', {
  ...
  globalThis.__FD_RUNTIME_SHELL_198__ = {
    version: VERSION,
    build: BUILD,
    state,
    install,
