...
(() => {
  ...
  const BUILD = 199;
  if (globalThis.__FD_RUNTIME_SHELL_199__) return;

  const root = document.documentElement;
  root.dataset.fdBuild = String(BUILD);
  root.dataset.fdVersion = VERSION;

  const boot = globalThis.__FD_BOOT_199__;
  const state = {
    installed: false,
    launching: false,
    launchMode: null,
  ...
  const showLaunchError = (text) => {
    ...
    console.error('[FD199] Launch error:', state.lastError);
  ...
  const safeSave190 = (reason = 'periodic') => {
    ...
    try {
      ...
      console.info('[FD199] Checkpoint saved:', reason);
      return true;
    } catch (error) {
      if (parseSave190(previous, keys.current)) storageSet190(api, keys.current, previous);
      console.warn('[FD199] Checkpoint save failed:', reason, error);
      return false;
    }
  };
  ...
  const finishSuccessfulLaunch = () => {
    ...
    boot?.stop?.();
    root.classList.remove('fd-loading199', 'fd-ready199');
    root.classList.add('fd-running199');
    state.launching = false;
    const mode = state.launchMode;
    state.launchMode = null;
    configureAutosave190();
    console.info(
      '[FD199] Stable game observed after launch',
  ...
  const launchNewGame = (event) => {
    ...
    console.info('[FD199] Starting new game', options);
    try {
      api.startGame(options);
      verifyLaunch();
    } catch (error) {
      console.error('[FD199] startGame threw', error);
  ...
  const launchSavedGame = (event) => {
    ...
    console.info('[FD199] Starting saved game from', candidate.key);
    try {
      api.startGame({
      ...
      console.error('[FD199] saved start threw', error);
  ...
  globalThis.__FD_RUNTIME_SHELL_199__ = {
    version: VERSION,
    build: BUILD,
    state,
    install,
