...
(() => {
  ...
  const BUILD = 201;
  if (globalThis.__FD_RUNTIME_SHELL_201__) return;

  const root = document.documentElement;
  root.dataset.fdBuild = String(BUILD);
  root.dataset.fdVersion = VERSION;

  const boot = globalThis.__FD_BOOT_201__;
  const state = {
    installed: false,
    launching: false,
    launchMode: null,
  ...
  const showLaunchError = (text) => {
    ...
    console.error('[FD201] Launch error:', state.lastError);
  ...
  const safeSave190 = (reason = 'periodic') => {
    ...
    try {
      ...
      console.info('[FD201] Checkpoint saved:', reason);
      return true;
    } catch (error) {
      if (parseSave190(previous, keys.current)) storageSet190(api, keys.current, previous);
      console.warn('[FD201] Checkpoint save failed:', reason, error);
      return false;
    }
  };
  ...
  const finishSuccessfulLaunch = () => {
    ...
    boot?.stop?.();
    root.classList.remove('fd-loading201', 'fd-ready201');
    root.classList.add('fd-running201');
    state.launching = false;
    const mode = state.launchMode;
    state.launchMode = null;
    configureAutosave190();
    console.info(
      '[FD201] Stable game observed after launch',
  ...
  const launchNewGame = (event) => {
    ...
    console.info('[FD201] Starting new game', options);
    try {
      api.startGame(options);
      verifyLaunch();
    } catch (error) {
      console.error('[FD201] startGame threw', error);
  ...
  const launchSavedGame = (event) => {
    ...
    console.info('[FD201] Starting saved game from', candidate.key);
    try {
      api.startGame({
      ...
      console.error('[FD201] saved start threw', error);
  ...
  const install = () => {
    ...
    console.info('[FD201] Canonical launch/save owner ready', {
  ...
  globalThis.__FD_RUNTIME_SHELL_201__ = {
    version: VERSION,
    build: BUILD,
    state,
    install,
