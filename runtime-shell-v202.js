...
(() => {
  ...
  const BUILD = 202;
  if (globalThis.__FD_RUNTIME_SHELL_202__) return;

  const root = document.documentElement;
  root.dataset.fdBuild = String(BUILD);
  root.dataset.fdVersion = VERSION;

  const boot = globalThis.__FD_BOOT_202__;
  const state = {
    installed: false,
    launching: false,
    launchMode: null,
  ...
  const showLaunchError = (text) => {
    ...
    console.error('[FD202] Launch error:', state.lastError);
  ...
  const safeSave190 = (reason = 'periodic') => {
    ...
    try {
      ...
      console.info('[FD202] Checkpoint saved:', reason);
      return true;
    } catch (error) {
      if (parseSave190(previous, keys.current)) storageSet190(api, keys.current, previous);
      console.warn('[FD202] Checkpoint save failed:', reason, error);
      return false;
    }
  };
  ...
  const finishSuccessfulLaunch = () => {
    ...
    boot?.stop?.();
    root.classList.remove('fd-loading202', 'fd-ready202');
    root.classList.add('fd-running202');
    state.launching = false;
    const mode = state.launchMode;
    state.launchMode = null;
    configureAutosave190();
    console.info(
      '[FD202] Stable game observed after launch',
  ...
  const launchNewGame = (event) => {
    ...
    console.info('[FD202] Starting new game', options);
    try {
      api.startGame(options);
      verifyLaunch();
    } catch (error) {
      console.error('[FD202] startGame threw', error);
  ...
  const launchSavedGame = (event) => {
    ...
    console.info('[FD202] Starting saved game from', candidate.key);
    try {
      api.startGame({
      ...
      console.error('[FD202] saved start threw', error);
  ...
  const install = () => {
    ...
    console.info('[FD202] Canonical launch/save owner ready', {
  ...
  globalThis.__FD_RUNTIME_SHELL_202__ = {
    version: VERSION,
    build: BUILD,
    state,
    install,
