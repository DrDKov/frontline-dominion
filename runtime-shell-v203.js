...
(() => {
  ...
  const BUILD = 203;
  if (globalThis.__FD_RUNTIME_SHELL_203__) return;

  const root = document.documentElement;
  root.dataset.fdBuild = String(BUILD);
  root.dataset.fdVersion = VERSION;

  const boot = globalThis.__FD_BOOT_203__;
  const state = {
    installed: false,
    launching: false,
    launchMode: null,
  ...
  const showLaunchError = (text) => {
    ...
    console.error('[FD203] Launch error:', state.lastError);
  ...
  const safeSave190 = (reason = 'periodic') => {
    ...
    try {
      ...
      console.info('[FD203] Checkpoint saved:', reason);
      return true;
    } catch (error) {
      if (parseSave190(previous, keys.current)) storageSet190(api, keys.current, previous);
      console.warn('[FD203] Checkpoint save failed:', reason, error);
      return false;
    }
  };
  ...
  const finishSuccessfulLaunch = () => {
    ...
    boot?.stop?.();
    root.classList.remove('fd-loading203', 'fd-ready203');
    root.classList.add('fd-running203');
    state.launching = false;
    const mode = state.launchMode;
    state.launchMode = null;
    configureAutosave190();
    console.info(
      '[FD203] Stable game observed after launch',
  ...
  const launchNewGame = (event) => {
    ...
    console.info('[FD203] Starting new game', options);
    try {
      api.startGame(options);
      verifyLaunch();
    } catch (error) {
      console.error('[FD203] startGame threw', error);
  ...
  const launchSavedGame = (event) => {
    ...
    console.info('[FD203] Starting saved game from', candidate.key);
    try {
      api.startGame({
      ...
      console.error('[FD203] saved start threw', error);
  ...
  const install = () => {
    ...
    console.info('[FD203] Canonical launch/save owner ready', {
  ...
  globalThis.__FD_RUNTIME_SHELL_203__ = {
    version: VERSION,
    build: BUILD,
    state,
    install,
