...
(() => {
  ...
  const BUILD = 205;
  if (globalThis.__FD_RUNTIME_SHELL_205__) return;

  const root = document.documentElement;
  root.dataset.fdBuild = String(BUILD);
  root.dataset.fdVersion = VERSION;

  const boot = globalThis.__FD_BOOT_205__;
  const state = {
    installed: false,
    launching: false,
    launchMode: null,
  ...
  const showLaunchError = (text) => {
    ...
    console.error('[FD205] Launch error:', state.lastError);
  ...
  const safeSave190 = (reason = 'periodic') => {
    ...
    try {
      ...
      console.info('[FD205] Checkpoint saved:', reason);
      return true;
    } catch (error) {
      if (parseSave190(previous, keys.current)) storageSet190(api, keys.current, previous);
      console.warn('[FD205] Checkpoint save failed:', reason, error);
      return false;
    }
  };
  ...
  const finishSuccessfulLaunch = () => {
    ...
    boot?.stop?.();
    root.classList.remove('fd-loading205', 'fd-ready205');
    root.classList.add('fd-running205');
    state.launching = false;
    const mode = state.launchMode;
    state.launchMode = null;
    configureAutosave190();
    console.info(
      '[FD205] Stable game observed after launch',
  ...
  const launchNewGame = (event) => {
    ...
    console.info('[FD205] Starting new game', options);
    try {
      api.startGame(options);
      verifyLaunch();
    } catch (error) {
      console.error('[FD205] startGame threw', error);
  ...
  const launchSavedGame = (event) => {
    ...
    console.info('[FD205] Starting saved game from', candidate.key);
    try {
      api.startGame({
      ...
      console.error('[FD205] saved start threw', error);
  ...
  const install = () => {
    ...
    console.info('[FD205] Canonical launch/save owner ready', {
  ...
  globalThis.__FD_RUNTIME_SHELL_205__ = {
    version: VERSION,
    build: BUILD,
    state,
    install,
