...
(() => {
  ...
  const BUILD = 206;
  if (globalThis.__FD_RUNTIME_SHELL_206__) return;

  const root = document.documentElement;
  root.dataset.fdBuild = String(BUILD);
  root.dataset.fdVersion = VERSION;

  const boot = globalThis.__FD_BOOT_206__;
  const state = {
    installed: false,
    launching: false,
    launchMode: null,
  ...
  const showLaunchError = (text) => {
    ...
    console.error('[FD206] Launch error:', state.lastError);
  ...
  const safeSave190 = (reason = 'periodic') => {
    ...
    try {
      ...
      console.info('[FD206] Checkpoint saved:', reason);
      return true;
    } catch (error) {
      if (parseSave190(previous, keys.current)) storageSet190(api, keys.current, previous);
      console.warn('[FD206] Checkpoint save failed:', reason, error);
      return false;
    }
  };
  ...
  const finishSuccessfulLaunch = () => {
    ...
    boot?.stop?.();
    root.classList.remove('fd-loading206', 'fd-ready206');
    root.classList.add('fd-running206');
    state.launching = false;
    const mode = state.launchMode;
    state.launchMode = null;
    configureAutosave190();
    console.info(
      '[FD206] Stable game observed after launch',
  ...
  const launchNewGame = (event) => {
    ...
    console.info('[FD206] Starting new game', options);
    try {
      api.startGame(options);
      verifyLaunch();
    } catch (error) {
      console.error('[FD206] startGame threw', error);
  ...
  const launchSavedGame = (event) => {
    ...
    console.info('[FD206] Starting saved game from', candidate.key);
    try {
      api.startGame({
      ...
      console.error('[FD206] saved start threw', error);
  ...
  const install = () => {
    ...
    console.info('[FD206] Canonical launch/save owner ready', {
  ...
  globalThis.__FD_RUNTIME_SHELL_206__ = {
    version: VERSION,
    build: BUILD,
    state,
    install,
