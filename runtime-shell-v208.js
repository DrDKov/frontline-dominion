...
(() => {
  ...
  const BUILD = 208;
  if (globalThis.__FD_RUNTIME_SHELL_208__) return;

  const root = document.documentElement;
  root.dataset.fdBuild = String(BUILD);
  root.dataset.fdVersion = VERSION;

  const boot = globalThis.__FD_BOOT_208__;
  const state = {
    installed: false,
    launching: false,
    launchMode: null,
  ...
  const showLaunchError = (text) => {
    ...
    console.error('[FD208] Launch error:', state.lastError);
  ...
  const safeSave190 = (reason = 'periodic') => {
    ...
    try {
      ...
      console.info('[FD208] Checkpoint saved:', reason);
      return true;
    } catch (error) {
      if (parseSave190(previous, keys.current)) storageSet190(api, keys.current, previous);
      console.warn('[FD208] Checkpoint save failed:', reason, error);
      return false;
    }
  };
  ...
  const finishSuccessfulLaunch = () => {
    ...
    boot?.stop?.();
    root.classList.remove('fd-loading208', 'fd-ready208');
    root.classList.add('fd-running208');
    state.launching = false;
    const mode = state.launchMode;
    state.launchMode = null;
    configureAutosave190();
    console.info(
      '[FD208] Stable game observed after launch',
  ...
  const launchNewGame = (event) => {
    ...
    console.info('[FD208] Starting new game', options);
    try {
      api.startGame(options);
      verifyLaunch();
    } catch (error) {
      console.error('[FD208] startGame threw', error);
  ...
  const launchSavedGame = (event) => {
    ...
    console.info('[FD208] Starting saved game from', candidate.key);
    try {
      api.startGame({
      ...
      console.error('[FD208] saved start threw', error);
  ...
  const install = () => {
    ...
    console.info('[FD208] Canonical launch/save owner ready', {
...
globalThis.__FD_RUNTIME_SHELL_205__ ||= globalThis.__FD_RUNTIME_SHELL_208__; globalThis.__FD_BOOT_205__ ||= globalThis.__FD_BOOT_208__;

globalThis.__FD_RUNTIME_SHELL_206__ ||= globalThis.__FD_RUNTIME_SHELL_208__; globalThis.__FD_BOOT_206__ ||= globalThis.__FD_BOOT_208__;
