...
(() => {
  ...
  const BUILD = 209;
  if (globalThis.__FD_RUNTIME_SHELL_209__) return;

  const root = document.documentElement;
  root.dataset.fdBuild = String(BUILD);
  root.dataset.fdVersion = VERSION;

  const boot = globalThis.__FD_BOOT_209__;
  const state = {
    installed: false,
    launching: false,
    launchMode: null,
  ...
  const showLaunchError = (text) => {
    ...
    console.error('[FD209] Launch error:', state.lastError);
  ...
  const safeSave190 = (reason = 'periodic') => {
    ...
    try {
      ...
      console.info('[FD209] Checkpoint saved:', reason);
      return true;
    } catch (error) {
      if (parseSave190(previous, keys.current)) storageSet190(api, keys.current, previous);
      console.warn('[FD209] Checkpoint save failed:', reason, error);
      return false;
    }
  };
  ...
  const finishSuccessfulLaunch = () => {
    ...
    boot?.stop?.();
    root.classList.remove('fd-loading209', 'fd-ready209');
    root.classList.add('fd-running209');
    state.launching = false;
    const mode = state.launchMode;
    state.launchMode = null;
    configureAutosave190();
    console.info(
      '[FD209] Stable game observed after launch',
  ...
  const launchNewGame = (event) => {
    ...
    console.info('[FD209] Starting new game', options);
    try {
      api.startGame(options);
      verifyLaunch();
    } catch (error) {
      console.error('[FD209] startGame threw', error);
  ...
  const launchSavedGame = (event) => {
    ...
    console.info('[FD209] Starting saved game from', candidate.key);
    try {
      api.startGame({
      ...
      console.error('[FD209] saved start threw', error);
  ...
  const install = () => {
    ...
    console.info('[FD209] Canonical launch/save owner ready', {
  ...
  globalThis.__FD_RUNTIME_SHELL_209__ = {
    version: VERSION,
    build: BUILD,
    state,
    install,
...
globalThis.__FD_RUNTIME_SHELL_205__ ||= globalThis.__FD_RUNTIME_SHELL_209__; globalThis.__FD_BOOT_205__ ||= globalThis.__FD_BOOT_209__;

globalThis.__FD_RUNTIME_SHELL_206__ ||= globalThis.__FD_RUNTIME_SHELL_209__; globalThis.__FD_BOOT_206__ ||= globalThis.__FD_BOOT_209__;

globalThis.__FD_RUNTIME_SHELL_207__ ||= globalThis.__FD_RUNTIME_SHELL_209__; globalThis.__FD_RUNTIME_SHELL_206__ ||= globalThis.__FD_RUNTIME_SHELL_209__; globalThis.__FD_BOOT_207__ ||= globalThis.__FD_BOOT_209__; globalThis.__FD_BOOT_206__ ||= globalThis.__FD_BOOT_209__;
