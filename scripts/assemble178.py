...
UA='Mozilla/5.0 FrontlineDominion-GitHubPages/178'
...
queue += [
 ...
 'combined-arms-doctrine-v141.js?build=166',
]
seen=set()
while queue:
...
s=re.sub(r'<title>.*?</title>','<title>Frontline Dominion v16.4.3 — Engineer Build Lock</title>',s,count=1,flags=re.S)
...
s=re.sub(r'((?:/|\./)?authoritative-simulation-v174\.js)\?build=\d+',r'\1?build=178',s)
s=re.sub(r'((?:/|\./)?multiplayer-game-v96\.js)\?build=\d+',r'\1?build=178',s)
s=re.sub(r'((?:/|\./)?combined-arms-doctrine-v141\.js)\?build=\d+',r'\1?build=178',s)
p.write_text(s,'utf-8')

p=OUT/'authoritative-simulation-v174.js'; s=p.read_text('utf-8')
s=re.sub(r"new Worker\('/authoritative-simulation-worker-v174\.js\?build=\d+'\)","new Worker('/authoritative-simulation-worker-v174.js?build=178')",s,count=1)
...
s=s.replace("/authoritative-simulation-bundle-v172.js?build=172", "/authoritative-simulation-bundle-v172.js?build=178")
...
# Build 178: explicit engineer build orders are non-evasive; minefields render only individual mines.
def patch_build178_module(path):
    p=OUT/path
    text=p.read_text('utf-8')
    engineer_old="""    if (isEngineer141(this)) {\n      if (processEngineerEvasion141(this, dt)) return;\n      if (command?.type === 'layMinefield141') { processEngineerMinefield141(this, command, dt); return; }\n"""
    engineer_new="""    if (isEngineer141(this)) {\n      const playerDirectedBuild178 = command?.type === 'build' && !command?.autoEngineer130;\n      if (!playerDirectedBuild178 && processEngineerEvasion141(this, dt)) return;\n      if (command?.type === 'layMinefield141') { processEngineerMinefield141(this, command, dt); return; }\n"""
    if engineer_old not in text:
        raise RuntimeError(f'v178 engineer patch anchor missing: {path}')
    text=text.replace(engineer_old, engineer_new, 1)
    circle_old="""      const center = this.worldToScreen(field.x, field.y, 0);\n      const edge = this.worldToScreen(field.x + field.radius, field.y, 0);\n      const screenRadius = Math.max(12, Math.hypot(edge.x - center.x, edge.y - center.y));\n      const friendly = field.team === 'player';\n      context.globalAlpha = friendly ? .72 : .58;\n      context.strokeStyle = friendly ? 'rgba(246,207,102,.82)' : 'rgba(255,117,98,.85)';\n      context.fillStyle = friendly ? 'rgba(246,207,102,.055)' : 'rgba(255,117,98,.055)';\n      context.lineWidth = 1.35;\n      context.setLineDash([7, 7]);\n      context.beginPath();\n      context.arc(center.x, center.y, screenRadius, 0, TWO_PI);\n      context.fill();\n      context.stroke();\n      context.setLineDash([]);\n      const stride = field.mines.length > 28 ? 2 : 1;\n"""
    circle_new="""      const friendly = field.team === 'player';\n      context.globalAlpha = friendly ? .90 : .80;\n      // v178: no minefield perimeter/radius overlay; render armed mines only.\n      const stride = 1;\n"""
    if circle_old not in text:
        raise RuntimeError(f'v178 minefield circle patch anchor missing: {path}')
    text=text.replace(circle_old, circle_new, 1)
    label_re=re.compile(r"\n      context\.globalAlpha = \.92;\n      context\.fillStyle = friendly \? '#f7dfa0' : '#ffc0af';\n      context\.font = '800 10px system-ui, sans-serif';\n      context\.textAlign = 'center';\n      context\.fillText\(`\$\{field\.mines\.length\}/\$\{field\.maxMines\} мин`, center\.x, center\.y - screenRadius - 8\);")
    text,n=label_re.subn('', text, count=1)
    if n != 1:
        raise RuntimeError(f'v178 minefield label patch anchor missing: {path}')
    p.write_text(text,'utf-8')

for module178 in ('combined-arms-doctrine-v141.js','authoritative-simulation-bundle-v172.js'):
    patch_build178_module(module178)
...
(OUT/'index.html').write_text('<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Frontline Dominion v16.4.3</title><meta http-equiv="refresh" content="0; url=./frontline-dominion.html?build=178"></head><body><a href="./frontline-dominion.html?build=178">Запустить Frontline Dominion v16.4.3</a></body></html>','utf-8')
(OUT/'.nojekyll').touch()
