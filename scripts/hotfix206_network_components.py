...
components = r"""  function canonicalLogisticsComponents206(game, perspectiveSwapped = false) {
    ...
    let teams = 2166136261 >>> 0;
    let nodes = 2166136261 >>> 0;
    let units = 2166136261 >>> 0;
    let missions = 2166136261 >>> 0;
    for (const canonical of ['player', 'enemy']) {
      const local = localTeam(canonical, perspectiveSwapped);
      teams = fnvText(teams, canonical);
      teams = fnvNumber(teams, game.teams?.[local]?.credits, 100);
      const teamState = state.team?.[local] || {};
      teams = fnvNumber(teams, teamState.supportSpent, 100);
      teams = fnvNumber(teams, teamState.importSpent, 100);
      const contracts = teamState.contracts || {};
      for (const id of Object.keys(contracts).sort()) {
        teams = fnvText(teams, id);
        const byResource = contracts[id] || {};
        for (const resource of ['fuel', 'ammo']) {
          const contract = byResource[resource] || {};
          teams = fnvText(teams, resource);
    ...
    for (const entity of stableEntities(game)) {
      ...
      if (entity.kind === 'building') {
        ...
        if (s?.stock) nodes = mixManifest(nodes, s.stock);
        if (s?.importBuffer) nodes = mixManifest(nodes, s.importBuffer);
        ...
        if (Number.isFinite(entity.resourceBuffer83)) nodes = fnvNumber(nodes, entity.resourceBuffer83);
        if (entity.resourceType206) nodes = fnvText(nodes, entity.resourceType206);
      } else {
        units = fnvText(units, entity.id); units = fnvText(units, canonicalTeam);
        if (s?.cargo) units = mixManifest(units, s.cargo);
        for (const key of ['fuel','fuelMax','ammoReady','ammoReadyMax','ammoReserve','ammoReserveMax','support','supportMax','routeRisk','reloadRemaining206']) {
          if (Number.isFinite(s?.[key])) units = fnvNumber(units, s[key]);
        }
        ...
        for (const key of ['missionType','status','phase206','sourceNodeId','destinationNodeId','homeNodeId','targetGroupId']) {
          if (s?.[key] != null) missions = fnvText(missions, s[key]);
        }
      }
    }
