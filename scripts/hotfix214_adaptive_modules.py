...
for path in (HTML, WORKER, AI_STRATEGY, AI_ADAPTIVE, ENGINEER):
    if not path.exists():
...
if html.count(ai_tag) != 1 or html.count(adaptive_tag) != 1 or html.count(engineer_tag) != 1:
...
if worker.count(ai_import) != 1 or worker.count(adaptive_import) != 1 or worker.count(engineer_import) != 1:
...
if manifest.exists():
    ...
    if 'adaptive-ai-logistics-strategy' not in text:
        text = text.replace('"ai-logistics-recovery"', '"ai-logistics-recovery", "adaptive-ai-logistics-strategy", "adaptive-ai-logistics-controller", "explicit-engineer-construction"')
        manifest.write_text(text, 'utf-8')
    elif 'adaptive-ai-logistics-controller' not in text:
        text = text.replace('"adaptive-ai-logistics-strategy"', '"adaptive-ai-logistics-strategy", "adaptive-ai-logistics-controller", "explicit-engineer-construction"')
        manifest.write_text(text, 'utf-8')

print('Build 214 adaptive AI logistics controller and explicit engineer construction authority injected into page and Worker')
