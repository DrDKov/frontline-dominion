...
root = Path('.')
out = root / 'dist'
modules = ['engineer-rocket-visual-v190.js', 'extractor-placement-v190.js', 'command-input-v190.js']
for name in modules:
...
extractor_tag = '<script src="./extractor-placement-v190.js?build=190"></script>'
...
for tag in (visual_tag, extractor_tag, input_tag):
...
if page.count(visual_tag) != 1 or page.count(extractor_tag) != 1 or page.count(input_tag) != 1:
    raise RuntimeError('runtime module ownership is not unique')
page_path.write_text(page, 'utf-8')
print('Build 190 runtime input, engineer visual and extractor placement owners installed')
