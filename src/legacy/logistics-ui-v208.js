(() => {
  ...
  const Game=D.Game;
  if(Game.prototype.__fdLogisticsUI208Installed)return;
  Object.defineProperty(Game.prototype,'__fdLogisticsUI208Installed',{value:true,configurable:true});
  ...
  function shell(layout){
    ...
    if(layout==='extractor')return `<h4 data-fd-title207>Добывающее предприятие</h4><div class="fd-grid207">${field('resourceType','Ресурс')}${field('deposit','Остаток месторождения')}${field('buffer','Локальный склад')}${field('rate','Скорость добычи')}${field('moneyIncome','Денежный доход')}${field('haul','Вывоз')}</div>`;
  ...
  function updatePanel(game){
    ...
    if(layout==='truck'){
      ...
      const ex=L.ensureExtractor(entity),node=game.getEntity?.(entity.resourceNodeId),resource=ex?.resourceType==='fuel'?'Fuel':'Железо / Ammo';const max=Number(node?.maxAmount207||node?.maxAmount)||0,current=Number(node?.amount)||0,bufferMax=Number(entity.resourceBufferMax206||entity.stats?.bufferCapacity)||0;const title=panel.querySelector('[data-fd-title207]');if(title)title.textContent=entity.stats?.name||'Добывающее предприятие';setField(panel,'resourceType',resource);setField(panel,'deposit',`${amount(current)} / ${amount(max)} (${pct(current,max)})`);setField(panel,'buffer',`${amount(entity.resourceBuffer83)} / ${amount(bufferMax)}`);setField(panel,'rate',`${amount(entity.stats?.extractPerTick||0)} ед./с`);setField(panel,'moneyIncome',`+${amount(entity.stats?.moneyIncomePerSecond208||0)} ¤/с`);setField(panel,'haul','грузовик автоматически · инженер только по приказу');
    }else if(layout==='node'){
  ...
  function handleAction(event){
    ...
    if(['area','group','building','extract'].includes(action)){beginTarget(game,action);return;}
    ...
    else if(action==='manual'&&L.isTruck(one)){beginTarget(game,'manual');return;}
  ...
  window.__FD_LOGISTICS_UI208__={build:207,version:'16.9.1',state,panel:()=>ensurePanel(),tab:()=>ensureTabs()?.tab||null,update:()=>D.game&&updatePanel(D.game),open(){state.activeTab='logistics';applyTabState207();updatePanel(D.game);}};
})();
