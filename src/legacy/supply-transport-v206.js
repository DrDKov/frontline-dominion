(() => {
  ...
  function availableAtSource206(source, key) {
    ...
    const typeStats=D.BUILDING_TYPES?.[source.typeId],tied=Boolean(source.stats?.tiedSupplyTransport208||typeStats?.tiedSupplyTransport208);const reserve=tied?Math.max(0,Number(source.game?.productionMaterialPackage206?.(source,'resourceTruck')?.[key])||0):0;return Math.max(0,(Number(node.stock?.[key])||0)-reserve);
  }
  ...
  function unitDeficit206(unit, excludeId=null) {
  ...
  function missionDemand206(game,truck) {
    ...
    if(s.missionType==='SUPPLY_AREA') return areaDemand206(game,truck.team,Number(s.targetX)||truck.x,Number(s.targetY)||truck.y,Math.max(320,Number(s.serviceRadius)||680),truck.id);
  ...
  function resupplyUnit206(game,truck,unit,dt,budget) {
    ...
    if(moved>EPS){s.resupplySourceId=truck.id;s.lastResupplyAt206=Number(game.time)||0;s.resupplyProgress=L.clamp(s.resupplyProgress+dt*.45,0,1);unit.supply160=L.unitReadiness(unit).supply;unit._fdLogisticsDetailUntil208=(Number(game.time)||0)+2.5;}
    return moved;
  }
  ...
  function chooseAutoDestination211(game,truck) {
    const s=L.ensureUnit(truck,false),candidates=[];
    for(const building of game.buildings||[]){
      if(!building?.alive||!building.completed||building.team!==truck.team)continue;
      const node=L.ensureNode(building);if(!node?.stock||L.ensureExtractor(building))continue;
  ...
  function processTruck206(game,truck,dt) {
    ...
    if(s.phase206==='LOAD') {
      ...
      const freshDemand=missionDemand206(game,truck);const rawDemand=L.manifestTotal(freshDemand)>1?freshDemand:(s.plannedDemand206||freshDemand);const demand=L.copyManifest(rawDemand);
  ...
  Game.prototype.setLogisticsMission206=function(payload={}){
    ...
    for(const id of ids){const truck=this.getEntity?.(id);if(!truck?.alive||!L.isTruck(truck))continue;const s=L.ensureUnit(truck,false);const mission=MISSIONS.includes(payload.missionType)?payload.missionType:'AUTO';const previousMission208=s.missionType;
      ...
      const nextDemand208=missionDemand206(this,truck),cargoTotal208=L.manifestTotal(s.cargo);
