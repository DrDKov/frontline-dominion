...
(() => {
  ...
  const BUILD = 187;
  if (globalThis.__FD_RUNTIME_SHELL_187__) return;
  ...
  const ownMetadata = () => {
    document.title = `Frontline Dominion v${VERSION} — Safari Startup Recovery`;
    const eyebrow = document.querySelector('#start-screen .eyebrow');
    if (eyebrow) eyebrow.textContent = `ОПЕРАТИВНО-ТАКТИЧЕСКАЯ RTS · v${VERSION} BUILD ${BUILD}`;
  };
  ...
  globalThis.__FD_RUNTIME_SHELL_187__ = {
    version: VERSION,
    build: BUILD,
    revealStart,
    ownMetadata,
