(() => {
  ...
  function fuelStoreAt207(entity){
    ...
    if(node?.stock&&Number(node.stock.fuelMax)>0)return {get:()=>Math.max(0,Number(node.stock.fuel)||0),set:v=>{node.stock.fuel=round(Math.max(0,v));}};
    return null;
  }
  function truckRefuel207(game,dt){
    for(const truck of game.units||[]){
      ...
      if(!(Number(s.fuelMax)>0))s.fuelMax=TRUCK_TANK;
