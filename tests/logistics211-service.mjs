...
await waitFor(() => {
  ...
  return b && !b.disabled && globalThis.__FD_LOGISTICS_INTEGRITY_211__ && globalThis.__FD_GAMEPLAY_208__ && globalThis.__FD_RUNTIME_SHELL_211__?.state?.installed ? true : null;
});
...
const fixture = await page.evaluate(I => {
  ...
  D.Game.prototype.initializeBattle = function(...args) {
    ...
    globalThis.__FD211_FIXTURE_TYPES__={vehicleType,infantryType,airType};
    return result;
  };
  return { ok: true };
}, I);
...
await waitFor(I => {
  ...
  return g?.getEntity?.(I.provider)?.alive && g?.getEntity?.(I.receiver)?.alive && b?.ready && !b.failed && Number(b.workerTick)>12
    ? { tick:b.workerTick, recoveries:Number(b.recoveryAttempts201||0), types:globalThis.__FD211_FIXTURE_TYPES__ }
...
// Area centre is 14600,14500. Receiver/tank/infantry are 480-520 away: old ~310 SERVICE radius cannot reach them.
...
const area = await waitFor(I => {
  ...
  const state=id=>{const u=g.getEntity(id),s=L.ensureUnit(u,false);return{id,fuel:Number(s?.fuel||0),fuelMax:Number(s?.fuelMax||0),ammo:Number(s?.ammoReserve||0),support:Number(s?.support||0),cargo:{...s?.cargo},resupplySourceId:s?.resupplySourceId||null};};
  ...
  if(receiver.fuel>100 && tank.fuel>100 && (infantry.ammo>10||infantry.support>10)) return {receiver,tank,infantry,outside,air,provider,tick:b.workerTick};
...
if(!(area.provider.cargo.fuel < initial.provider.cargo.fuel || area.provider.resupplySourceId===null)) {
  // Provider began empty and loads dynamically; conservation is checked below instead of requiring a positive initial cargo.
}

// Explicitly verify the direct-service target cap on a recipient whose tank started empty.
if(area.receiver.fuel < area.receiver.fuelMax*.80) throw new Error(`receiver truck tank insufficiently refuelled ${JSON.stringify(area.receiver)}`);

// Group centre members are ~360 units from the prescribed rear standoff, so old 310 service radius fails.
...
if(!final.marker?.truckToTruckTankService || !final.marker?.missionRadiusAuthoritative || !final.marker?.receiverCargoIsolation) throw new Error(`build211 marker missing ${JSON.stringify(final.marker)}`);
