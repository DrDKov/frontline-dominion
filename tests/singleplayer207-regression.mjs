...
const menu=await waitFor(()=>{const slots=globalThis.__FD_SAVE_SLOTS_207__,shell=globalThis.__FD_RUNTIME_SHELL_207__;return slots?.state?.ready&&shell?.state?.installed&&globalThis.__FD_SINGLEPLAYER_207__&&globalThis.__FD_LOGISTICS_UI207__?{build:slots.build}:null;},undefined,30000);
if(menu.build!==207)throw new Error(`Wrong build owners: ${JSON.stringify(menu)}`);
...
const panelReady=await waitFor(()=>{const panel=document.getElementById('fd-logistics-panel207'),up=panel?.querySelector('[data-fd-action207="priority-up"]'),create=panel?.querySelector('[data-fd-action207="create-transport"]');if(!panel?.classList.contains('active')||!up||!create)return null;globalThis.__fdButtonIdentity207={up,create};return{priority:panel.querySelector('[data-fd-field207="priority"]')?.textContent};});
await page.waitForTimeout(1400);
const stable=await page.evaluate(()=>({up:globalThis.__fdButtonIdentity207?.up===document.querySelector('[data-fd-action207="priority-up"]'),create:globalThis.__fdButtonIdentity207?.create===document.querySelector('[data-fd-action207="create-transport"]')}));
if(!stable.up||!stable.create)throw new Error(`Logistics controls were recreated during Worker snapshots: ${JSON.stringify(stable)}`);
...
const truck=await waitFor(()=>{const game=globalThis.__FD_DEBUG__?.game,u=(game?.units||[]).find(x=>x?.alive&&x.team==='player'&&x.typeId==='resourceTruck');if(!u)return null;game.setSelection([u],false);game.renderSelectionUI();return{id:u.id,fuel:u.logistics206?.fuel,fuelMax:u.logistics206?.fuelMax,capacity:u.logistics206?.cargoCapacity};},undefined,45000);
if(!(truck.fuelMax>0)||!(truck.capacity>=6000))throw new Error(`Truck tank/capacity missing: ${JSON.stringify(truck)}`);
const truckUi=await waitFor(()=>{const text=document.getElementById('fd-logistics-panel207')?.textContent||'';return /Топливо в баке/.test(text)&&/Груз \/ вместимость/.test(text)?text:null;});

// Extractor details expose finite reserve, local buffer and extraction rate when an extractor exists.
const extractorUi=await page.evaluate(()=>{const game=globalThis.__FD_DEBUG__.game,L=globalThis.__FD_LOGISTICS206__,b=game.buildings.find(x=>x?.alive&&x.team==='player'&&L.ensureExtractor(x));if(!b)return{present:false};game.setSelection([b],false);game.renderSelectionUI();return{present:true,id:b.id,name:b.stats?.name};});
if(extractorUi.present){const text=await waitFor(()=>{const t=document.getElementById('fd-logistics-panel207')?.textContent||'';return /Остаток месторождения/.test(t)&&/Локальный склад/.test(t)&&/Скорость добычи/.test(t)?t:null;});if(!text)throw new Error('Extractor capacity details missing');}

// Aircraft control regression: if player aircraft are present, a normal user move must not crash/recover Worker or collapse unit mirror.
const airFixture=await page.evaluate(()=>{const game=globalThis.__FD_DEBUG__.game,bridge=globalThis.__FD_STABLE_STATE165__.bridge;const air=(game.units||[]).find(u=>u?.alive&&u.team==='player'&&(u.air||u.stats?.air));if(!air)return{present:false,units:game.units.length};game.setSelection([air],false);const beforeSeq=Number(bridge.seq||0),beforeTick=Number(bridge.workerTick||0),recoveries=Number(bridge.recoveryAttempts201||0),units=game.units.filter(u=>u?.alive).length;const issued=game.issueMove(air.x+420,air.y+180,false);return{present:true,id:air.id,issued,beforeSeq,sent:Number(bridge.seq||0),beforeTick,recoveries,units};});
if(airFixture.present){if(!airFixture.issued||airFixture.sent<=airFixture.beforeSeq)throw new Error(`Aircraft move did not enter Worker: ${JSON.stringify(airFixture)}`);await waitFor(seq=>Number(globalThis.__FD_STABLE_STATE165__?.bridge?.lastAck||0)>=seq,airFixture.sent,15000);await page.waitForTimeout(3500);const after=await page.evaluate(f=>{const game=globalThis.__FD_DEBUG__.game,b=globalThis.__FD_STABLE_STATE165__.bridge;return{ready:b.ready,failed:b.failed,tick:Number(b.workerTick||0),recoveries:Number(b.recoveryAttempts201||0),units:game.units.filter(u=>u?.alive).length,airAlive:Boolean(game.getEntity(f.id)?.alive),lastError:b.lastError||null,lastRecoveryReason:b.lastRecoveryReason201||null};},airFixture);if(!after.ready||after.failed||after.tick<=airFixture.beforeTick||after.recoveries!==airFixture.recoveries||after.units<Math.max(1,airFixture.units-2)||!after.airAlive)throw new Error(`Aircraft command destabilized Worker/world: ${JSON.stringify({airFixture,after})}`);}

if(errors.length)throw new Error(`Browser errors: ${JSON.stringify(errors)}`);
console.log(JSON.stringify({ok:true,menu,ready,resourceModel:{visibleTypes:resourceModel.visibleTypes,names:resourceModel.names,resourceCount:resourceModel.resources.length},panelReady,stable,priorityRoundTrip:true,createSeq,truck,truckUi:true,extractorUi,airFixture}));
