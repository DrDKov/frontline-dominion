...
const result=await evalJs(`(()=>{let stage='begin';try{const D=window.__FD_DEBUG__,g=D.game,b=window.__FD_STABLE_STATE165__?.bridge;stage='find-building';const building=g.buildings.find(x=>x?.alive&&x.team==='player'&&x.completed!==false&&(x.stats?.produces||[]).length);if(!building)return {stage,error:'no-production-building'};const itemId=building.stats.produces[0];stage='select';for(const e of g.selected||[])e.selected=false;g.selected=[building];building.selected=true;g.uiDirty=true;stage='updateUI';g.updateUI?.();stage='buttons';const buttons=[...document.querySelectorAll('button')].map((el,index)=>({index,text:(el.textContent||'').trim().replace(/\\s+/g,' ').slice(0,180),disabled:!!el.disabled,id:el.id||'',className:String(el.className||''),dataset:Object.fromEntries(Object.entries(el.dataset||{}))}));return {stage:'ok',building:{id:building.id,typeId:building.typeId,itemId,queue:(building.queue||[]).length},bridge:{seq:b?.seq,lastAck:b?.lastAck,actionErrors:b?.actionErrors},buttons:buttons.slice(-60)};}catch(e){return {stage,error:String(e?.message||e),stack:String(e?.stack||'')};}})()`);
console.log('UI_DIAGNOSTIC '+JSON.stringify(result));
ws.close();
