...
page.on('console', message => { if (['error', 'warning'].includes(message.type())) consoles.push({ type: message.type(), text: message.text() }); });
...
const initial = await page.evaluate(() => {
  ...
  return {
    ...
    start: rect(start),
    button: { ...rect(button), disabled: !!button?.disabled },
    shell187: !!globalThis.__FD_RUNTIME_SHELL_187__,
    ui187: !!globalThis.__FD_RUNTIME_UI_187__,
    ui186: !!globalThis.__FD_RUNTIME_UI_186__,
...
if (!initial.button || initial.button.display === 'none' || initial.button.visibility === 'hidden' || Number(initial.button.opacity) <= 0 || initial.button.width < 20 || initial.button.height < 20 || initial.button.disabled) {
  throw new Error(`WebKit start button is not actionable: ${JSON.stringify(initial.button)}`);
}
...
await page.locator('#start-game').click({ timeout: 5000 });
await page.waitForFunction(() => !!globalThis.__FD_DEBUG__?.game && Number(globalThis.__FD_STABLE_STATE165__?.bridge?.workerTick || 0) > 0, { timeout: 15000 });
...
const state = await page.evaluate(() => {
  ...
  try {
    ...
    for (let i = 0; i < d.length; i += 16) {
      const r = d[i], gg = d[i + 1], bb = d[i + 2], v = Math.max(r, gg, bb);
      if (v > 12) lit += 1; max = Math.max(max, v); sum += v; bins.add((r >> 4) + ',' + (gg >> 4) + ',' + (bb >> 4));
    }
  ...
  const rect = element => {
    if (!element) return null;
    const r = element.getBoundingClientRect(), s = getComputedStyle(element);
    return { x: r.x, y: r.y, width: r.width, height: r.height, display: s.display, visibility: s.visibility, opacity: s.opacity, z: s.zIndex };
  };
  return {
    href: location.href,
    title: document.title,
    ...
    selection: rect(document.getElementById('selection-panel')),
    action: rect(document.getElementById('action-panel')),
    start: rect(document.getElementById('start-screen')),
    center: document.elementsFromPoint(innerWidth / 2, innerHeight / 2).slice(0, 8).map(el => el.id || String(el.className) || el.tagName),
  };
});
...
for (let i = 0; i < 30; i += 1) {
  titleSamples.push(await page.title());
